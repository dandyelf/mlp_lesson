...
void MlpMainWin::BlinkLogic() {
    ...
    if(!set_data_1_) button_blink_ = ui_->button1_set_data;
    else if (!load_data_2_) button_blink_ = ui_->button2_load_data;
    else if (!add_tests_3_) button_blink_ = ui_->button3_add_tests;
    else if (!load_tests_4_) button_blink_ = ui_->button4_load_tests;
    else button_blink_ = ui_->button5_start_edu;
}
